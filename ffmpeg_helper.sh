...
configure_ffmpeg()
{
	...
	current_dir=$PWD
	cd ffmpeg
	CFLAGS="$CFLAGS $INCLUDES" LDFLAGS="" ./configure \
...
download_ffmpeg()
{
	if [ -d ffmpeg/.git ]; then
		echo ffmpeg has already been downloaded
		return 0
	else
...
{
	echo Running make
	make -j`nproc` -C ffmpeg
	return $?
}

install_ffmpeg()
{
	echo Copying compiled ffmpeg files to ffmpeg_compiled
	make -C ffmpeg -j9 DESTDIR="../ffmpeg_compiled" install
	strip ffmpeg_compiled/usr/local/lib/*.so
}

if [ "$1" = "all" ]; then
	download_ffmpeg && configure_ffmpeg && make_ffmpeg && install_ffmpeg
	...
	print_and_run make -C ffmpeg clean
else
	usage
fi
